...
| F12 | 统计报表 | 中 | 已实现 | 10 种图表（饼图/柱状图/折线图）+ 饼图 hover 全名提示 + PDF 明细报表导出 |
...
| department | VARCHAR(50) | NOT NULL | 适用部门 |
...
| `src/tabs/DashboardTab.h/cpp` | 仪表盘：6 张统计卡片 + 合同到期提醒 + 10 种图表 + 饼图 hover 明细 + PDF 明细报表导出 |
...
*本文档基于对全部源码文件的审查编写。V3.3 更新于 2026-05-29，反映当前代码库完整功能状态（18 项功能、17 张数据表、10 种统计图表、8 项侧边栏导航、含可折叠侧边栏、岗位薪资标准、审批意见留痕、可视化组织架构图、QODBC 连接隔离、公共 UI 样式工具与客户端服务层重构）。*
