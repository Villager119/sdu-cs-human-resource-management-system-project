...
| F9 | 组织架构管理 | 中 | 已实现 | 部门层级管理、负责人指派、部门员工列表；包含拖拽缩放的交互式可视化树状图 |
...
- 假设 `hrms_db` 数据库已创建，16 张表通过启动迁移自动创建
...
| 审批中心-请假审批 | QTableView | 待审批请假单列表，含同意/拒绝按钮 |
| 审批中心-补卡审批 | QTableView | 待审批补卡单列表，含同意/拒绝按钮 |
| 同意按钮 | QPushButton | 仅 admin 可见，同意后更新对应记录并发送通知 |
| 拒绝按钮 | QPushButton | 仅 admin 可见，拒绝后发送通知 |
...
**功能**：
- **双视图呈现**：支持“部门结构列表”与“可视化架构图”选项卡视图的自由切换。
- **列表视图**：
  - 左侧部门树：层级展示所有部门及当前在职员工数量。
  - 右侧员工列表：选中部门后显示该部门所属的所有在职员工。
- **可视化视图**：
...
| `src/tabs/DashboardTab.h/cpp` | 仪表盘：6 张统计卡片 + 合同到期提醒 + 10 种图表 + PDF 导出 |
...
| `src/tabs/AttendManageTab.h/cpp` | 考勤管理：考勤面板 + 审批中心（请假审批 + 补卡审批） |
