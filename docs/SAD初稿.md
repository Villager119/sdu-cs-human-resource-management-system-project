...
HRMS 是一套基于 **C++/Qt6/MySQL** 的桌面端企业级人力资源管理系统，主要包括：员工档案CRUD与批量调整、岗位职称薪资标准校验、考勤打卡、补卡与请假审批、多维度绩效评分、一键自动薪酬核算、按姓名查询工资条、组织架构树呈现、多维统计报表、未读通知推送以及全局安全操作审计日志等功能。该系统部署于局域网或单机环境，不面向高并发、外网或移动端场景。
...
- **[U-1] 可用性与容错能力 (Usability & Reliability)**：防误操作（支持事务、批量撤销和切页未保存提醒），防止因未选中行引发的非法指针崩溃。
...
| **薪酬核算大盘** | `calculate_payroll` | 可用（“一键核算全员薪资”、按姓名查询工资条） | **直接隐藏** | 屏蔽核算按钮和姓名查询框，员工仅能看个人历史工资条 |
...
- **架构决策**：系统在保存连接参数时，对密码等敏感字符串执行 Base64 编码，再存入 `config.ini` 文件中。读取时再由程序解码还原；为兼容历史配置，启动和服务器设置页同时支持 `Database/PWD` 与 `Database/Password` 两种键名。
- **决策合理性**：
