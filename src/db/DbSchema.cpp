...
bool initDatabaseSchema()
{
    ...
    q.finish();
    if (employeesEmpty) {
        q.prepare("INSERT INTO employees (name, phone, role, password_hash, hire_date, base_salary, status, shift_id) "
                  "VALUES ('admin', '13800000000', 'admin', '25f43b1486ad95a1398e3eeb3d83bc4010015fcc9bedb35b432e00298d5021f7', ?, 8000.00, '在职', 1)");
    ...
    // 19. Migration: add unique phone per employee
    {
        bool uniquePhoneExists = false;
        {
            QSqlQuery ck;
            if (ck.exec("SHOW KEYS FROM employees WHERE Key_name = 'uk_employees_phone'") && ck.next()) {
                uniquePhoneExists = true;
            }
            ck.finish();
        }

        if (!uniquePhoneExists) {
            bool duplicatePhoneExists = false;
            {
                QSqlQuery ck;
                if (ck.exec("SELECT phone, COUNT(*) FROM employees "
                            "WHERE phone IS NOT NULL GROUP BY phone HAVING COUNT(*) > 1 LIMIT 1") && ck.next()) {
                    duplicatePhoneExists = true;
                    qWarning() << "Skip adding uk_employees_phone to employees because duplicate phone exists"
                               << ck.value(0).toString()
                               << "count" << ck.value(1).toInt();
                }
                ck.finish();
            }
            ...
            if (!duplicatePhoneExists) {
                QSqlQuery q;
                if (!q.exec("ALTER TABLE employees ADD UNIQUE KEY uk_employees_phone (phone)")) {
                    qDebug() << "Failed to add unique key uk_employees_phone to employees:" << q.lastError().text();
                    db.rollback();
                    return false;
                }
                q.finish();
    ...
    // 20. Migration: add unique score per employee per evaluation month
    {
    ...
    // 21. Migration: add unique payroll record per employee per month
    {
        bool uniqueKeyExists = false;
        {
            QSqlQuery ck;
    ...
    // 22. Migration: add reviewer metadata to approval request tables
    ...
    for (const QString &table : approvalTables) {
        if (!ensureDbColumn(table, "reviewer_id", "INT DEFAULT NULL") ||
            !ensureDbColumn(table, "reviewed_at", "DATETIME DEFAULT NULL") ||
            !ensureDbColumn(table, "review_comment", "VARCHAR(255) DEFAULT ''")) {
            db.rollback();
            return false;
        }
    }

    if (!ensureDbColumn("employees", "shift_id", "INT DEFAULT 1")) {
        db.rollback();
        return false;
    }
    {
        QSqlQuery q;
        if (!q.exec("UPDATE employees SET shift_id=1 WHERE shift_id IS NULL")) {
            qDebug() << "Failed to backfill employees.shift_id:" << q.lastError().text();
            q.finish();
            db.rollback();
            return false;
        }
    ...
    if (!ensureDbIndex("employees", "idx_employees_status_hire", "status, hire_date") ||
        !ensureDbIndex("attendances", "idx_attendances_date_status", "att_date, status") ||
        !ensureDbIndex("leave_requests", "idx_leave_emp_status_dates", "emp_id, status, start_date, end_date") ||
        !ensureDbIndex("makeup_requests", "idx_makeup_emp_date_type_status", "emp_id, att_date, request_type, status") ||
        !ensureDbIndex("profile_change_requests", "idx_profile_change_emp_field_status", "emp_id, field_name, status") ||
        !ensureDbIndex("performance_scores", "idx_perf_emp_month_status", "emp_id, eval_month, status")) {
        db.rollback();
        return false;
    }

    if (!db.commit()) {
        qDebug() << "Failed to commit database schema initialization:" << db.lastError().text();
        return false;
    }
